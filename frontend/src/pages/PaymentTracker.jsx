...
export default function PaymentTracker(){
  ...
  function add(){
    setEditId(null);setVendorChoice("");setForm(initial());setBillFile(null);setShowMore(false);setOpen(true)
  }
  ...
  function validate(){
    ...
    for(const x of form.LineItems){
      if(num(x.TaxableAmount)>0&&!x.LedgerID)throw new Error(`Select Ledger / Account Head for ${x.Particulars||"each line"}`);
    }
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:"1rem"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:"1rem",flexWrap:"wrap"}}>
      <div>
        ...
        <p style={{color:"var(--muted)",fontSize:".8rem"}}>Vendor bills, GST/TDS/TCS, exact round-off and accounting-linked payments</p>
      </div>
    ...
    <div style={card}>
      <input style={input} placeholder="Search vendor, bill no or description" value={search} onChange={e=>setSearch(e.target.value)}/>
    </div>
    ...
    <div style={{...card,padding:0,overflowX:"auto"}}>
      <table>
        <thead>
          <tr>
            <th>Vendor</th><th>Bill</th><th>Date</th><th>Gross</th><th>TCS</th><th>TDS</th><th>Round Off</th><th>Net</th><th>Paid</th><th>Balance</th><th>Status</th><th></th>
          </tr>
        </thead>
        <tbody>
          ...
          {!filtered.length&&<tr><td colSpan="12" style={{padding:"2rem",textAlign:"center"}}>No bills</td></tr>}
        </tbody>
      </table>
    </div>
        ...
        <form onSubmit={save}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))",gap:".75rem"}}>
            ...
            {vendorChoice==="__new__"&&<>
              <div><label style={label}>New Vendor Name *</label><input style={input} required value={form.VendorName} onChange={e=>set("VendorName",e.target.value)}/></div>
              <div><label style={label}>Category</label><select style={input} value={form.VendorCategory} onChange={e=>set("VendorCategory",e.target.value)}>{CATEGORIES.map(x=><option key={x}>{x}</option>)}</select></div>
            </>}
            ...
            <div style={{gridColumn:"1/-1",border:"1px solid var(--border)",borderRadius:"6px",padding:".8rem"}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:".6rem"}}>
                ...
                <button type="button" style={btn("ghost")} onClick={()=>setForm(p=>({...p,LineItems:[...p.LineItems,emptyLine()]}))}>+ Add line</button>
              </div>
            ...
            <div>
              ...
              <input style={input} type="number" min="0" step=".01" value={form.TCSAmount} onChange={e=>set("TCSAmount",e.target.value)} placeholder="TCS charged by vendor"/>
              <small style={{color:"var(--muted)"}}>Enter TCS separately exactly as shown on the vendor invoice.</small>
            </div>
            ...
            <div style={{gridColumn:"1/-1",fontWeight:700}}>
              Payment Details <span style={{fontWeight:400,color:"var(--muted)",fontSize:".72rem"}}>— optional; leave Paid Amount blank for an unpaid bill</span>
            </div>
            ...
            {a.paid>0&&<>
              ...
              {!personal(form.PaidByType)&&norm(form.PaymentMode)!=="cash"&&<>
                <div>
                  ...
                  {selectedBank&&<small style={{color:"var(--muted)"}}>Current {fmt(selectedBank.CurrentBalance)} · After payment {fmt(num(selectedBank.CurrentBalance)-a.paid)}</small>}
                </div>
                <div><label style={label}>Reference / UTR *</label><input style={input} required value={form.ReferenceNo} onChange={e=>set("ReferenceNo",e.target.value)}/></div>
              </>}

              {personal(form.PaidByType)&&<>
                <div><label style={label}>Actual Paid By *</label><select style={input} required value={form.PaidByName} onChange={e=>setForm(p=>({...p,PaidByName:e.target.value,ReimburseTo:p.ReimburseTo||e.target.value}))}><option value="">— Select person —</option>{partnerNames.map(x=><option key={x}>{x}</option>)}</select></div>
          ...
          <div style={{display:"flex",justifyContent:"flex-end",gap:".6rem",marginTop:"1rem"}}>
            ...
            <button type="submit" style={btn()} disabled={saving||!!duplicate}>{saving?"Saving...":editId?"Update Bill":"Save Bill"}</button>
          </div>
        </form>
      </div>
    </div>}
